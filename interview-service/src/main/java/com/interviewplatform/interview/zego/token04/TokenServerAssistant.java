package com.interviewplatform.interview.zego.token04;
...
/**
 * Token generation utility copied from ZEGOCLOUD/zego_server_assistant
 * (https://github.com/ZEGOCLOUD/zego_server_assistant, token/java/token04/
 * src/im/zego/serverassistant/utils/TokenServerAssistant.java).
 *
 * <p>All cryptographic and byte-packing logic is kept verbatim from ZEGO's
 * official implementation (AES-256-CBC, random 16-byte IV, nonce, and the
 * [expire + ivLen + iv + cipherLen + ciphertext] layout before Base64 with the
 * "04" version flag). The only deviation from the original is the JSON
 * construction, which here uses Jackson's {@link ObjectMapper} instead of
 * org.json.simple to build the same encrypted plain-text object with the same
 * keys and value types (app_id=number, user_id=string, nonce=number,
 * ctime=number, expire=number, payload=string).
 */
